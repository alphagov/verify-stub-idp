...
public class ConsentResource {
    ...
    public Response get(
        ...
        IdpUser idpUser = session.get().getIdpUser().get();

        List<AuthnContext> requestLevelsOfAssurance = session.get().getIdaAuthnRequestFromHub().getLevelsOfAssurance();
    ...
    public Response consent(
        ...
        switch (submitButtonValue) {
            case I_AGREE_SUBMIT_VALUE:
                return samlResponseRedirectViewFactory.sendSamlMessage(successAuthnResponseService.getSuccessResponse(randomisePid, "TODO: Get IP ADDRESS from request", idpName, session.get()));
            case I_REFUSE_SUBMIT_VALUE:
                return samlResponseRedirectViewFactory.sendSamlMessage(nonSuccessAuthnResponseService.generateNoAuthnContext(session.get().getIdaAuthnRequestFromHub().getId(), idpName, session.get().getRelayState()));
