...
public class SecurityHeadersFilterTest {
    ...
    public static void checkSecurityHeaders(MultivaluedMap<String, Object> headers) {
        ...
        assertThat(headers.get("Content-Security-Policy").get(0)).isEqualTo("default-src 'self'; font-src 'data:'; img-src 'self'; object-src 'none'; script-src 'self' 'unsafe-inline'; style-src 'self' 'unsafe-inline'");
    }
